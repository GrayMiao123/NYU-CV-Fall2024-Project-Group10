...
if __name__ == '__main__':
    ...
    prune_channels = np.array(prune_channels)
    channels_dis = np.array(channels_dis)
    ...
    plt.tight_layout()
    plt.savefig('channels_chart.png')
    print('save to channels_chart.png')
    
    # 将原始数据写入CSV文件
    with open('channels_data.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['name', 'base_channels', 'prune_channels'])
        for n, b, p in zip(names, base_channels, prune_channels):
            writer.writerow([n, b, p])
    print('save to channels_data.csv')
    
    # 按照 channels_dis 排序的数据可视化及CSV导出
    channels_dis_sort = np.argsort(channels_dis)[::-1]
    base_channels_sort = base_channels[channels_dis_sort]
    prune_channels_sort = prune_channels[channels_dis_sort]
    names_sort = names[channels_dis_sort]
    
    x = np.arange(len(names_sort))
    plt.figure(figsize=(25, 8))
    plt.xticks(x, names_sort, rotation=90)
    plt.bar(x, base_channels_sort, color='orange', label='base')
    plt.bar(x, prune_channels_sort, color='red', label='prune')
    plt.legend(fontsize=20)
    plt.title('Channel contrast diagram (sorted)', fontdict={'fontsize':20})
    plt.tight_layout()
    plt.savefig('channels_chart_sort.png')
    print('save to channels_chart_sort.png')
    
    # 将排序后数据写入CSV文件
    with open('channels_data_sort.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['name', 'base_channels', 'prune_channels'])
        for n, b, p in zip(names_sort, base_channels_sort, prune_channels_sort):
            writer.writerow([n, b, p])
    print('save to channels_data_sort.csv')
